<script lang="ts">
  ...
  const dispatch = createEventDispatcher(),
    startGame = () => {
      ...
      board = new Array(10)
        ...
        .map(() =>
          new Array(10)
            .fill(0)
            .map(() => ({ isOpen: false, isFlagged: false, value: Math.random() > 0.85 ? -1 : 0 }))
        );
      for (let i = 0; i < board.length; i++)
        for (let j = 0; j < board[i].length; j++) {
          if (board[i][j].value === -1) continue;
          board[i][j].value = 0;
          for (let ii = Math.max(0, i - 1); ii < Math.min(board.length, i + 2); ii++)
            for (let jj = Math.max(0, j - 1); jj < Math.min(board[i].length, j + 2); jj++)
    ...
    openCell = (i: number, j: number) => {
      ...
      if (board.every(row => row.filter(c => c.value !== -1).every(c => c.isOpen)))
        isGameFinished = true;
    };
